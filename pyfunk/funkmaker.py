...
def compose():
    ...
    for i in range(length):
        ...
        if ((x%2 == 0 and random.randint(0, 3) == 0) or x == 0) and mode == 1 and len(progression) > 0:
    ...
    while True:
        if len(progressions) < 4:
            ...
            for x in range(1, 4):
                ...
                # Sometimes the HookTheory APK doesn't pass a valid value, so nothing else I can do but except the error ¯\_(ツ)_/¯
                ...
                for i in range(len(options)):
                    # No idea where these errors come from but excepting them works fine
                    # We break the loop because otherwise it will thow this error many more times in a row (still works though, but though this was probably better)
                    try:
                        if i == 0:
                            options[i] = [[0, receivedchord[i]['probability']], receivedchord[i]['chord_ID']]
                        elif i == len(options)-1:
                            options[i] = [[options[i-1][0][1], 1], receivedchord[i]['chord_ID']]
                        else:
...
def translatechord(chrd):
    ...
    if isinstance(finalchord, list):
        ...
        print('error:', finalchord)

thread.start_new_thread(HookTheoryGet, ())
while len(progressions) < 4:
    print('Waiting for HookTheory (%d%%)...' % (len(progressions*25)))
    time.sleep(2)
