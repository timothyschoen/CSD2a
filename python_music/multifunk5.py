...
import random, sys, time, sched, rtmidi
import _thread as thread
from midiutil.MidiFile import MIDIFile

#Open 4 midi outs, put them in an array for easy access
...
def compose():
    ...
    for i in range(length):
        ...
        if  x == 0 and mode == 1:
            ...
            change = 1
        #Generate Chords off scale
        elif (x%2 == 0 and random.randint(0, 2) == 0) and mode == 1:
            if negative == 1:
                negative = 0
                scale = [0, 2, 4, 5, 7, 9, 11]
                scale = spreadNotes(scale)
            ...
            parts[4][i] = [movement(scalepos+60, scale, 0), movement(scalepos+60, scale, 2), movement(scalepos+60, scale, 4), movement(scalepos+60, scale, 6)]
            change = 1
        elif (x%4 == 0 and random.randint(0, 10) == 0) and mode == 1:
            scale = [0, 2, 3, 5, 7, 8, 10]
            scale = spreadNotes(scale)
            scale = [(x+key) for x in scale]
            options = [[4], [0, 5], [6], [0, 5], [4, 6], [3, 1], [4, 6]][chordchoice]
            chordchoice = random.choice(options)
            scalepos = scale[chordchoice]
            parts[4][i] = [movement(scalepos+60, scale, 0), movement(scalepos+60,scale, 2), movement(scalepos+60, scale, 4), movement(scalepos+60, scale, 6)]
            change = 1
