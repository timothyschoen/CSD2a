...
lastnotes = 10*list([currentnotes])
lastall = [[48, 60, 67, 72]]
choice = [0, 0]
    ...
    for seqs in range(len(sequence)):
        ...
        for i in range(len(addedlist)):
            ...
            for y in [0, 2]:
                ...
                for x in range(8):
                    if lastchoices[x][int(y/2)] >= 2 or lastchoices[x][int(y/2)] <= -2 or (lastchoices[x][int(y/2)] == 1 and lastchoices[x-1][int(y/2)] == -1):
                        breaks.insert(0, 1)
                    else:
                        breaks.insert(0, 0)
        ...
        if currentnotes[2] - currentnotes[0] >= 12:
            if currentnotes[2] - currentnotes[1] > currentnotes[1] - currentnotes[0]:
                currentnotes[2] = currentnotes[2] - 12
            else:
                currentnotes[0] = currentnotes[0] + 12
        currentnotes.sort()
        ...
        if currentnotes[2] - currentnotes[0] == 9 and currentnotes[1] - currentnotes[0] == 5 and modulating == 2 and seqs < 14:
            ...
            allnotes = [bassnote] + currentnotes
        allnotes.sort()
        lastall.insert(0, list(allnotes))
        rounds = rounds+1
        if  modulating > 2:
            modulating = modulating - 1
            ...
            modulating = 25
            sequence[seqs] = allnotes
    # turn the chordlike notation into 4 individual voices, only add notes when they change
    seperatevoices = [[] for x in range(4)]
    for x in range(4):
        for y in range(16):
            timing = random.choice([0, 0, 0, 0.5, 0.5])
            if y > 0:
                if sequence[y][x] == sequence[y-1][x]:
                    pass
                elif len(sequence[y]) == 5:
                    seperatevoices[x].insert(0, [y, sequence[y][x]])
                else:
                    seperatevoices[x].insert(0, [y+timing, sequence[y][x]])
            else:
                seperatevoices[x].insert(0, [y, sequence[y][x]])
        seperatevoices[x].sort()
